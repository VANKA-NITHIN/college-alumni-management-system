...
/*--------------------------------------
    Premium Modern UI Animations
--------------------------------------*/
$(document).ready(function () {
    // 1. Dashboard Stat Counter Animation
    $('.total_no').each(function () {
        var $this = $(this);
        var targetVal = parseInt($this.text().trim(), 10);
    ...
    $('.main_bt, .btn-primary, .btn').on('mousedown', function () {
        $(this).css('transform', 'scale(0.96)');
    }).on('mouseup mouseleave', function () {
        $(this).css('transform', '');
    });

    // 3. Smooth page reveal
    $('body').css({ opacity: 0, transition: 'opacity 0.4s ease-in-out' }).css('opacity', 1);

    // 4. Form inputs focus state effects
    $('.form-control').on('focus', function () {
        $(this).closest('.field').find('label.label_field').css('color', 'var(--accent-color)');
    }).on('blur', function () {
        $(this).closest('.field').find('label.label_field').css('color', '');
    });

    // 5. Dynamic Active Sidebar State
    ...
    $('#sidebar ul li a').each(function () {
        ...
        if (href === currentPath) {
            ...
            var $parentCollapse = $link.closest('ul.collapse');
            if ($parentCollapse.length) {
                $parentCollapse.addClass('show');
                $parentCollapse.closest('li').addClass('active');
